...
Counts <- CountsALL %>%
  ...
  mutate(Method = factor(Method),
         ...
         Ord = case_when(Family == "Other Families" ~ 1,
                         TRUE ~ Prop)) %>%
  mutate(Family = fct_reorder(Family, Ord, .desc = T))
```
...
#Graphs - Pie charts
p1 <- Counts %>% ggplot(aes(x = factor(1), stat = "bin", fill = Family))+
  ...
  theme(text = element_text(family = "sans", size = 14, vjust = 0.75),
                            legend.position = "none")

# Graphs - Bar plots
# p1 <- ggplot(Counts %>% arrange(Family, Method, desc(Ord)), 
#          aes(x = Method, y = Prop, fill = Family))+
...
#Graph - Pie charts
p2 <- CountsBioM %>% ggplot(aes(x = factor(1), stat = "bin", fill = Family))+
  geom_bar(position = "fill", color = "white")+
  facet_grid(facets = Method ~ Bioregion)+
  coord_polar(theta="y")+
  theme_void()+
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#b2df8a", "#fb9a99", "#33a02c", "#e31a1c", 
                               "#ff7f00", "#fdbf6f", "#cab2d6", "#6a3d9a"),
                    guide = guide_legend(nrow = 4))+
  theme(text = element_text(family = "sans", size = 14, vjust = 0.75))

#Graph - Bar plots
# p2 <- ggplot(CountsBioM %>% arrange(Family, Method, Bioregion, desc(Ord)),
#          aes(x = "", y = Prop, fill = Family))+
#   geom_bar(position = "stack", stat = "identity")+
...
#Graph - Pie charts
p3 <- CountsBio %>% ggplot(aes(x = factor(1), stat = "bin", fill = Family))+
  geom_bar(position = "fill", color = "white")+
  facet_grid(facets = . ~ Bioregion)+
  coord_polar(theta="y")+
  theme_void()+
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c", "#fb9a99", "#e31a1c", 
                               "#b2df8a", "#cab2d6", "#6a3d9a"))+
  theme(text = element_text(family = "sans", size = 14, vjust = 0.75),
                            legend.position = "none")

#Graph - Bar plots
# p3 <- ggplot(CountsBio %>% arrange(Family, Bioregion, desc(Ord)),
#          aes(x = Bioregion, y = Prop, fill = Family))+
#   geom_bar(position = "stack", stat = "identity")+
...
CommComp <- ggarrange(plot_grid(p2leg, plot_grid(p1, p2+theme(legend.position = "none"), 
                                     ...
                                     hjust = -2.5, rel_widths = c(0.8, 1)), 
                    ncol = 1, rel_heights = c(0.6, 1, 0.7),
          p3, labels = c("", "", "C"), nrow = 3, hjust = -3))
CommComp

ggsave("Figures/CommunityFamCompAllPtsPies.tiff", CommComp, device = "tiff", dpi = 500)
...
MetFig <- ggarrange(Molfax_figM$graph, Mugil_figM$graph, Lutjanus_figM$graph, ncol = 3, 
                    common.legend = T, hjust = -1.75, legend = "bottom")
...
FinalFig
ggsave("Figures/LengthsKDE2.tiff", FinalFig, device = "tiff", dpi = 600)
